import ExperienceStep from "@/app/experience/page";
import { InitialData } from "@/app/models";
import { ResumeDataContext } from "@/app/services/form-context";
import '@testing-library/jest-dom'
import { fireEvent, render, screen, waitFor } from "@testing-library/react";
import { useRouter } from "next/navigation";

jest.mock('next/navigation', () => ({
    useRouter: jest.fn(),
}))

const mockPush = jest.fn()
const mockRouter = {
    push: mockPush,
    replace: jest.fn(),
    refresh: jest.fn(),
    back: jest.fn(),
    forward: jest.fn(),
    prefetch: jest.fn(),
}
const renderExperienceStepComponent = () => {
    const setResumeData = jest.fn();
    const resumeData = InitialData;
    render(<ResumeDataContext value={{ resumeData, setResumeData }}>
        <ExperienceStep />
    </ResumeDataContext>);
};
describe('Experience Step', () => {
    beforeEach(() => {
        (useRouter as jest.Mock).mockReturnValue(mockRouter);
    });
    it('Checks if the Experience Step component is rendered', () => {
        renderExperienceStepComponent();
        const experienceStep = screen.getByText('Add Experience');
        expect(experienceStep).toBeInTheDocument();
    });
    it('Checks if the step header is rendered', () => {
        renderExperienceStepComponent();
        expect(screen.getByText('Step 4: Add Your Experience')).toBeInTheDocument();
    });
    it('Title field is rendered', () => {
        renderExperienceStepComponent();
        expect(screen.getByLabelText('Title')).toBeInTheDocument();
    });
    it('On click of the Add Experience button it should add a new experience entry', async () => {
        renderExperienceStepComponent();
        const addExperienceButton = screen.getByText('Add Experience');
        fireEvent.click(addExperienceButton);
        expect(await waitFor(() => screen.getAllByLabelText('Company'))).toHaveLength(2);
    });
    it('Checks for the removal of an experience entry', async () => {
        renderExperienceStepComponent();
        const addExperienceButton = screen.getByText('Add Experience');
        fireEvent.click(addExperienceButton);
        await waitFor(() => fireEvent.click(addExperienceButton));
        const removeExperienceButton = screen.getAllByText('Remove');
        fireEvent.click(removeExperienceButton[0]);
        expect(await waitFor(() => screen.getAllByLabelText('Company'))).toHaveLength(2);
    });
    it('Back button is rendered and enabled', () => {
        renderExperienceStepComponent();
        expect(screen.getByText('Back')).toBeInTheDocument();
        expect(screen.getByText('Back')).toBeEnabled();
    });
    it('Next button is rendered and enabled', () => {
        renderExperienceStepComponent();
        expect(screen.getByText('Next')).toBeInTheDocument();
        expect(screen.getByText('Next')).toBeEnabled();
    });
    it('Save & Continue field should lead to experience route on successful submission', async () => {
        renderExperienceStepComponent();
        const saveButton = screen.getByText('Save & Continue');
        fireEvent.click(saveButton);
        expect(await waitFor(() => expect(mockRouter.push).toHaveBeenCalledWith('/summary')));
    });
});